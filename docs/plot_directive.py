    ...
    for i, code_piece in enumerate(code_pieces):
        ...
        for j in xrange(1000):
            if len(code_pieces) > 1:
                img = ImageFile(
                    basename="%s_%02d_%02d" % (output_base, i, j), dirname=output_dir
                )
            else:
                img = ImageFile(
                    basename="%s_%02d" % (output_base, j), dirname=output_dir
                )
            for format, dpi in formats:
                if out_of_date(code_path, img.filename(format)):
                    all_exists = False
                    ...
                    if config.plot_preserve_dir and outname:
                        print(
                            "Preserving '{0}' into '{1}'".format(
                                img.filename(format), config.plot_preserve_dir
                            )
                        )
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    if config.plot_preserve_dir:
        config.plot_preserve_dir = os.path.join(
