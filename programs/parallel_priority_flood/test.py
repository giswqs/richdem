...
def FillAndTest(
    authoritative, n, many_or_one, strat, inpfile: str, width=-1, height=-1
):
    ...
    print(
        "\033[96m###{strat} {manyone}: {width}x{height}###\033[39m".format(
            strat=strat if strat[0] == "@" else "@cache",
            manyone=many_or_one,
            width=width,
            height=height,
        )
    ...
    ######################
    output, err = doRaw(
        """mpirun -n {n} ./parallel_pf.exe {manyone} {strat} {inpfile} temp/manycore-%n.tif -w {width} -h {height}""".format(
            n=n,
            manyone=many_or_one,
            strat=strat,
            inpfile=inpfile,
    ...
    if err != 0:
        print(
            "\033[91mError in fill! Tiles probably too small; does not indicate a serious issue.\033[39m".format()
        )
        return
    ...
    output, err = doRaw(
        """gdal_merge.py -o temp/manycore_merged.tif -of GTiff -ot {dtype} -n {nodata} -a_nodata {nodata} {outfiles}""".format(
            dtype=dtype, nodata=nodata, outfiles=" ".join(outfiles)
        )
    )
    if err != 0:
        print("\033[91mError in merge!\033[39m".format())
        return
    ...
    ######################
    output, err = doRaw(
        """gdal_calc.py --NoDataValue=-9999 -A {authoritative} -B temp/manycore_merged.tif --calc="abs(A-B)" --outfile=temp/diff.tif""".format(
            authoritative=authoritative
        )
    )
    if err != 0:
        print("\033[91mError in diff!\033[39m".format())
        return
    ...
    if err != 0:
        print("\033[91mError in gdalinfo!\033[39m".format())
        return
...
def main():
    ...
    action.add_argument(
        "--many",
        action="store_true",
        help="Test a multiple file DEM. A layout file is expected.",
    )
    action.add_argument(
        "--one",
        action="store_true",
        help="Test a single file DEM. A GDAL readable DEM is expected.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Print all output information: use if something goes wrong.",
    )
    parser.add_argument(
        "--evict",
        action="store_true",
        help="Test the EVICT strategy. (Default is to test all.)",
    )
    parser.add_argument(
        "--cache",
        action="store_true",
        help="Test the CACHE strategy. (Default is to test all.)",
    )
    parser.add_argument(
        "--retain",
        action="store_true",
        help="Test the RETAIN strategy. (Default is to test all.)",
    )
    parser.add_argument(
        "--cores",
        action="store",
    ...
    parser.add_argument(
        "inputfile", help="Layout or DEM file", type=lambda x: is_valid_file(parser, x)
    )
    ...
    if args.many and not os.path.exists("temp/merged.tif"):
        ...
        with open(args.inputfile) as fin:
            ...
            filetiles = [
                os.path.join(os.path.dirname(args.inputfile), x) for x in filetiles
            ]
            ...
            output, err = doRaw(
                """gdal_merge.py -o temp/merged.tif -of GTiff \\
                             -ot {dtype} -n {nodata} -a_nodata {nodata} \\
                             {filetiles}""".format(
                    dtype=dtype, nodata=nodata, filetiles=" ".join(filetiles)
                )
            )
            if err != 0:
                print("Error merging!")
    ...
    if not os.path.exists("temp/auth.tif"):
        ...
        output, err = doRaw(
            "./auth_gen.exe {file} temp/auth.tif".format(file=auth_input)
        )
        ...
        if err != 0:
            ...
            sys.exit(-1)
        print(
            "Took {0:.4}s to generate authoritative answer.".format(
                stop_auth_gen - start_auth_gen
            )
        )
    else:
