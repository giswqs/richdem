...
def FillAndTest(
    ...
    ######################
    output, err = doRaw(
        """mpirun -n {n} ./parallel_d8_accum.exe {manyone} {strat} {inpfile} temp/manycore-%n.tif -w {width} -h {height}""".format(
            n=n,
            manyone=many_or_one,
            strat=strat,
            inpfile=inpfile,
    ...
    output, err = doRaw(
        """../../apps/rd_merge_rasters_by_layout.exe {layoutfile} 30 {outputfile} noflip""".format(
...
def main():
    ...
    if args.many and not os.path.exists("temp/merged.tif"):
        ...
        with open(args.inputfile) as fin:
            ...
            output, err = doRaw(
                """./merge_rasters_by_layout.exe {layoutfile} 30 {outputfile} noflip""".format(
                    layoutfile=args.inputfile, outputfile="temp/merged.tif"
                )
            )
            if err != 0:
                print("Error merging!")
    ...
    if not os.path.exists("temp/auth.tif"):
        ...
        output, err = doRaw(
            "../../apps/rd_flow_accumulation.exe {file} temp/auth.tif noflip".format(
                file=auth_input
            )
        )
